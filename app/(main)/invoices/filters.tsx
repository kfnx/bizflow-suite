...
export function Filters({ onFiltersChange, initialFilters }: FiltersProps) {
  ...
  const handleStatusChange = useCallback(
    (value: string) => {
      handleFiltersChange({ status: value as InvoiceStatus, page: 1 });
    },
    [handleFiltersChange],
  );
        ...
        <div className='flex items-center gap-2'>
          ...
          <Select.Root value={filters.sortBy} onValueChange={handleSortChange}>
            ...
            <Select.Content>
              ...
              <Select.Item value='due-date-asc'>
                Due Date (Earliest)
              </Select.Item>
              <Select.Item value='due-date-desc'>Due Date (Latest)</Select.Item>
              <Select.Item value='total-asc'>Total (Low to High)</Select.Item>
              <Select.Item value='total-desc'>Total (High to Low)</Select.Item>
            </Select.Content>
          </Select.Root>
        </div>
