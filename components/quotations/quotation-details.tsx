...
function DetailCard({
  ...
  return (
    <div
      className={`rounded-lg border border-stroke-soft-200 bg-bg-white-0 p-6 ${className}`}
    >
      <div className='mb-4 flex items-center gap-3'>
        ...
        <h3 className='text-lg font-semibold text-text-strong-950'>{title}</h3>
      </div>
      {children}
    </div>
  );
...
function DetailItem({
  ...
  return (
    <div className='flex items-start justify-between py-2'>
      ...
      <div className='text-sm text-right text-text-strong-950'>{value}</div>
    </div>
  );
}
    ...
    <div className='grid grid-cols-1 gap-6 lg:grid-cols-2'>
      ...
      <DetailCard icon={RiUserLine} title='Customer Information'>
        <div className='space-y-1'>
          <DetailItem label='Name' value={quotation.customerName} />
          <DetailItem label='Code' value={quotation.customerCode} />
