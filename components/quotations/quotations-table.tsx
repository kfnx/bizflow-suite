...
function ActionCell({
  ...
  const handleSubmit = async () => {
    if (
      ...
      row.original.status !== QUOTATION_STATUS.REVISED
    ) {
    ...
    <Dropdown.Root>
      ...
      <Dropdown.Content>
        ...
        <Dropdown.Item
          ...
          disabled={
            row.original.status !== 'draft' && row.original.status !== 'revised'
          }
        >
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>
        <div className='text-paragraph-sm text-text-sub-600'>
          {[
            row.original.createdByUserFirstName,
            row.original.createdByUserLastName,
          ]
            .filter(Boolean)
            .join(' ') || '—'}
        </div>
